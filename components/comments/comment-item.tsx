...
function authorName(comment: CommentViewModel) {
  if (comment.author.deletedAt) return "Удалённый пользователь";
          ...
          <div className="min-w-0 flex-1 space-y-2">
            <div className="flex min-w-0 flex-col gap-1 sm:flex-row sm:flex-wrap sm:items-center">
              {comment.author.deletedAt || isHidden ? (
                ...
                >
                  <PremiumName user={comment.author} className="truncate">
                    {name}
                  </PremiumName>
                </Link>
              )}
              ...
              {comment.status !== "PUBLISHED" ? (
                <span className="tk-pill px-1.5 py-0.5 text-[11px]">{comment.status}</span>
              ) : null}
            </div>
              ...
              {currentUser?.emailVerified ? (
                ...
                <span className="text-xs text-muted-foreground">
                  {currentUser
                    ...
                    : "Войдите, чтобы ответить или поставить реакцию."}
                </span>
              )}
            </div>
          </div>
