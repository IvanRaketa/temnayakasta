...
function authorName(comment: CommentViewModel) {
  return comment.author.deletedAt ? "Удалённый пользователь" : comment.author.profile?.displayName || comment.author.username;
}

function authorAvatar(comment: CommentViewModel) {
  return comment.author.profile?.avatar || comment.author.avatar;
}
...
function refreshPost(router: ReturnType<typeof useRouter>) {
  router.refresh();
...
function ReactionSubmitButton({
  ...
  return (
    <Button
      ...
      className="h-8 px-2 transition-transform active:scale-95"
      aria-label={type === "LIKE" ? "Лайк комментария" : "Дизлайк комментария"}
    >
...
function CommentReactionForm({
  ...
  return (
    <form action={action}>
      ...
      <ReactionSubmitButton type={type} count={count} active={active} disabled={disabled} />
    </form>
  );
}
...
function DeleteCommentForm({ slug, commentId }: DeleteCommentFormProps) {
  const router = useRouter();
      ...
      <div className="tk-glass rounded-md p-3 transition-colors sm:p-4">
        <div className="flex min-w-0 gap-3">
          <div className="tk-avatar-ring grid size-9 shrink-0 place-items-center overflow-hidden rounded-full bg-secondary text-xs font-semibold text-secondary-foreground">
            {avatar && !isHidden ? <img src={avatar} alt="" className="size-full object-cover" decoding="async" /> : name.slice(0, 2).toUpperCase()}
          </div>
          <div className="min-w-0 flex-1 space-y-2">
            <div className="flex min-w-0 flex-col gap-1 sm:flex-row sm:flex-wrap sm:items-center">
              {comment.author.deletedAt || isHidden ? (
                <span className="truncate text-sm font-medium text-foreground">{name}</span>
              ) : (
                <Link href={`/profile/${comment.author.username}`} className="inline-flex max-w-full items-center gap-1.5 text-sm font-medium text-foreground hover:text-primary">
                  <PremiumName user={comment.author} className="truncate">{name}</PremiumName>
                </Link>
              )}
            ...
            <p className={cn("whitespace-pre-wrap break-words text-sm leading-6 [overflow-wrap:anywhere]", isHidden && "text-muted-foreground")}>
              {isHidden ? DELETED_COMMENT_TEXT : comment.content}
            </p>
              ...
              {currentUser?.emailVerified ? (
                <>
                  ...
                  <CommentReactionForm
                    ...
                    commentId={comment.id}
                    type="DISLIKE"
                    count={comment.reactionCounts.DISLIKE}
                    active={comment.viewerReaction === "DISLIKE"}
                    disabled={!canInteract}
                  />
                  {comment.status === "PUBLISHED" ? (
                    <details className="group w-full sm:w-auto">
                      <summary className="inline-flex h-8 cursor-pointer list-none items-center justify-center gap-2 rounded-md px-2 text-sm font-medium transition-colors hover:bg-secondary hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring [&::-webkit-details-marker]:hidden">
                        <MessageSquareReply className="size-4" />
                        Ответить
                      </summary>
                      <div className="mt-2 origin-top transition-all duration-200 sm:min-w-96">
                        <ReplyForm slug={slug} parentId={comment.id} onDone={() => setRepliesOpen(true)} />
                      </div>
                    </details>
                  ) : null}
                  {isAuthor && !isHidden ? <DeleteCommentForm slug={slug} commentId={comment.id} /> : null}
                  {!isAuthor && !isHidden && !isPending ? <ReportForm targetType="COMMENT" targetId={comment.id} returnPath={postPath} className="w-full sm:w-auto" /> : null}
                </>
              ) : (
                <span className="text-xs text-muted-foreground">
                  {currentUser ? "Подтвердите e-mail для полного доступа." : "Войдите, чтобы ответить или поставить реакцию."}
                </span>
              )}
            </div>
          </div>
      ...
      {canToggleReplies ? (
        <div className="flex items-center">
          ...
          >
            ...
            <span className="truncate">{repliesOpen ? `Скрыть ветку · ${replyCountText}` : `Показать ветку · ${replyCountText}`}</span>
          </Button>
        </div>
      ) : null}
      {showReplies ? (
        <div id={repliesId} className={cn("relative space-y-3", depth < MAX_INDENT_LEVEL && "border-l border-border pl-3 sm:ml-5 sm:pl-4", depth >= MAX_INDENT_LEVEL && "pl-0")}>
