          ...
          OR: viewerId
            ? [
                { status: CommentStatus.PUBLISHED },
                {
                  authorId: viewerId,
                  status: {
                    in: [CommentStatus.PENDING_REVIEW, CommentStatus.HIDDEN, CommentStatus.BLOCKED],
                  },
                },
              ]
            : [{ status: CommentStatus.PUBLISHED }],
...
function createReactionSummary(
  ...
  return reactions.reduce<PostReactionSummary>(
    (summary, reaction) => {
      summary.counts[reaction.type] += 1;

      if (reaction.userId === viewerId) {
        summary.viewerReaction = reaction.type;
      }
...
function toDiscussionUser(
  user: NonNullable<Awaited<ReturnType<typeof getCurrentSessionReadOnly>>>["user"],
): DiscussionUser {
  return {
    id: user.id,
    ...
    profile: user.profile
      ? {
          ...
          premiumNameEffect: user.profile.premiumNameEffect,
        }
      : null,
  };
}
  ...
  for (const comment of comments) {
    ...
    byId.set(comment.id, {
      ...
      author: {
        ...
        premiumUntil: comment.author.premiumUntil?.toISOString() ?? null,
        deletedAt: comment.author.deletedAt?.toISOString() ?? null,
        profile: comment.author.profile
          ? {
              displayName: comment.author.profile.displayName,
              avatar: comment.author.profile.avatar,
              premiumNameEffect: comment.author.profile.premiumNameEffect,
            }
          : null,
      },
      reactionCounts: reactionSummary.counts as Record<ReactionTypeValue, number>,
...
export default async function PostPage({ params }: PostPageProps) {
  ...
  if (!post || !canViewPost(post, current?.user.id)) {
    notFound();
  }

  const publicSlug = getPostPublicSlug(post);
  if (slug !== publicSlug) {
    redirect(`/post/${publicSlug}`);
  }

  const isAuthor = post.authorId === current?.user.id;
  const authorName = getAuthorName(post);
  const authorAvatar = post.author.profile?.avatar || post.author.avatar;
  const authorPremiumActive = Boolean(
    post.author.premiumUntil && post.author.premiumUntil > new Date(),
  );
        ...
        <Card className="overflow-hidden p-5 md:p-7">
          <div className="space-y-5">
            <div className="flex flex-wrap items-center gap-2">
              {isAuthor ? <Badge variant="outline">{post.status}</Badge> : null}
              {authorPremiumActive && post.author.profile?.pinnedPostId === post.id ? (
            ...
            <h1 className="break-words text-3xl font-semibold leading-tight text-foreground md:text-5xl">
              {post.title}
            </h1>
            ...
            <div className="flex min-w-0 items-center gap-3 border-y border-border py-4">
              <div className="grid size-11 shrink-0 place-items-center overflow-hidden rounded-full bg-secondary text-sm font-semibold text-secondary-foreground">
                {authorAvatar ? (
                  ...
                  <img src={authorAvatar} alt="" className="size-full object-cover" decoding="async" />
                ) : (
              ...
              <div className="min-w-0">
                {post.author.deletedAt ? (
                  ...
                  >
                    <PremiumName user={post.author} className="truncate">
                      {authorName}
                    </PremiumName>
                  </Link>
                )}
            ...
            <div
              className="prose-tk max-w-none break-words"
              ...
              <div className="flex flex-wrap gap-2 border-t border-border pt-4">
                {post.tags.map(({ tag }) => (
                  <Link
                    key={tag.slug}
                    href={`/tag/${tag.slug}`}
                    className="max-w-full rounded-md border border-border bg-background px-2 py-1 text-xs text-muted-foreground transition hover:border-ring hover:text-foreground"
                  >
                    <span className="block max-w-48 truncate">#{tag.name}</span>
                  </Link>
                ))}
              </div>
            ) : null}
            ...
            <div className="border-t border-border pt-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                <ReportForm
                  ...
                  targetId={post.id}
                  returnPath={postPath}
                  disabled={!current?.user.emailVerified || isAuthor}
